...
def main():
    ...
    # Build logs exactly once to avoid accidental double-log transforms.
    log_ff_sr = np.log(df_AR['FF_SR'].to_numpy(dtype='float32'))
    log_ff_dr = np.log(df_AR['FF_DR'].to_numpy(dtype='float32'))

    # --- training target: log(FF_SR / FF_DR) = log(FF_SR) - log(FF_DR)
    delta_raw_np = log_ff_sr - log_ff_dr
    delta_clip_mask = (delta_raw_np < TARGET_CLIP_MIN) | (delta_raw_np > TARGET_CLIP_MAX)
    n_delta_clipped = int(delta_clip_mask.sum())
    if n_delta_clipped > 0:
        logger.info(
            'Clipping %d/%d target delta values to [%.1f, %.1f]',
            n_delta_clipped,
            len(delta_raw_np),
            TARGET_CLIP_MIN,
            TARGET_CLIP_MAX,
    ...
    delta_np = np.clip(delta_raw_np, TARGET_CLIP_MIN, TARGET_CLIP_MAX)
    target_log_ratio = t.tensor(delta_np, dtype=t.float32).unsqueeze(1)
    ...
    even_model, even_ckpt, even_logs, even_scaler_meta = _build_and_train_single_fold(
        fold_name='fold_odd',
        ff_target=target_log_ratio[odd_mask],
    ...
    odd_model, odd_ckpt, odd_logs, odd_scaler_meta = _build_and_train_single_fold(
        fold_name='fold_even',
        ff_target=target_log_ratio[even_mask],
    ...
    scaler_meta = {
        ...
        'schema': 'fold_combined_correction_flow_v1',
        'use_log_transform': True,
        'target_space': 'delta_log_ratio',
        'target_definition': 'log(FF_SR/FF_DR)',
        'target_clip_min': TARGET_CLIP_MIN,
        'target_clip_max': TARGET_CLIP_MAX,
        'condition_definition': 'event_var routing + (log(FF_DR) + variables_correction)',
        ...
        'n_target_delta_clipped': n_delta_clipped,
