...
class Args(Tap):
    ...
    output_root_base: str = 'Training_results_MC_even'  # Base directory where training folders are written.
...
def main():
    ...
    model_root_dir = Path(args.output_root_base) / mode_dir / f"training_{training_variables_name}"
    ...
    if 'event_var' not in data_complete.columns:
        raise KeyError("Column 'event_var' is required to enforce train/apply split, but is missing.")

    n_all_events = len(data_complete)
    n_event_var_0 = int((data_complete['event_var'] == 0).sum())
    n_event_var_1 = int((data_complete['event_var'] == 1).sum())

    logger.info(
        "Loaded %d total events from data_complete.feather (event_var==0: %d, event_var==1: %d).",
        n_all_events,
        n_event_var_0,
        n_event_var_1,
    )
    ...
    for spec in process_specs:
        if spec.dr_mask_name == 'mask_DR_wjets':
            # DR branch (AR-like/SR-like): use full dataset.
            data_for_spec = data_complete
            logger.info(
                "%s uses full dataset for training (event_var==0 and event_var==1): %d events.",
                spec.name,
                len(data_for_spec),
            )
        else:
            # antiDR branch: keep strict split to avoid train/apply overlap.
            data_for_spec = data_complete[data_complete['event_var'] == 0].copy()
            if not (data_for_spec['event_var'] == 0).all():
                raise RuntimeError("antiDR training dataframe contains events with event_var != 0.")
            logger.info(
                "%s uses event_var==0 split for training: %d events.",
                spec.name,
                len(data_for_spec),
            )
