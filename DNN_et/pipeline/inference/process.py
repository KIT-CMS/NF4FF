"""Process and classic-fraction fake-factor inference."""
...
from data.handling import FeatureRegistry, FeatureStore, load_data, load_variables
from inference.legacy import calculate_fake_factor_classic
from models.networks import load_model
from core.groupings import GROUPING_NAMES, grouping_suffix
...
def calculate_and_store_process_fake_factors(
    *,
    ...
    """Evaluate process FF models and persist row-index keyed features."""
...
def calculate_and_store_classic_fraction_fake_factors(
    *,
    data_path: Union[str, Path],
    masks_path: Union[str, Path],
    process_feature_path: Union[str, Path],
    feature_store_path: Union[str, Path],
    feature_registry_path: Union[str, Path],
    ...
    """Combine stored process FFs using correctionlib classic fractions."""
    process_fractions_path = Path(process_fractions_path)
    if not process_fractions_path.is_file():
        raise FileNotFoundError(
            f"Missing process-fraction correction file: {process_fractions_path}"
        )
    try:
        import correctionlib as cr
    except ImportError as error:
        raise ImportError(
            "Classic-fraction FF calculation requires correctionlib."
        ) from error

    df = load_data(data_path, masks_path)
    process_features = pd.read_feather(process_feature_path).set_index("row_index")
    selected = df.mask("SR") | df.mask("AR")
    frame = df.events.loc[selected].copy()
    process_columns = [
        _feature_name(process, grouping)
        for process in PROCESSES
        for grouping in groupings
    ]
    missing = sorted(set(process_columns).difference(process_features.columns))
    if missing:
        raise KeyError(f"Process FF feature file is missing columns: {missing}")
    frame.loc[:, process_columns] = process_features.reindex(frame.index)[
        process_columns
    ]
    ...
    for grouping in groupings:
        output_name = calculate_fake_factor_dnn(frame, grouping, fractions)
        output[output_name] = frame[output_name].to_numpy()
    if feature_suffix:
