"""Logging setup and contextual rich rendering."""
...
def setup_logging(
    ...
    # Prevent duplicate console output when another library already configured
    # the root logger (e.g. default StreamHandler/basicConfig).
    # Keep only handlers managed by this logging module.
    managed_types = (CustomRichHandler, RichRotatingFileHandler, logging.handlers.QueueHandler)
    for h in list(root.handlers):
        if not isinstance(h, managed_types):
