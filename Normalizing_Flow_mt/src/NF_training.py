...
def main():
    ...
    if args.njets == "all":
        ...
        events_np = events_np[njets_np == 0]
        weights_np = weights_np[njets_np == 0]
    elif args.njets == '1':
        events_np = events_np[njets_np == 1]
        weights_np = weights_np[njets_np == 1]
    elif args.njets == '2':
        events_np = events_np[njets_np >= 2]
        weights_np = weights_np[njets_np >= 2]
    else:
        raise ValueError(f"{args.njets} no valid input")

    # --- data splitting ---

    events_train_np, events_val_np, weights_train_np, weights_val_np = train_test_split(
        events_np,
        weights_np,
        test_size=0.2,
        random_state=42
    ...
    paths_training = StorePathHelper(directory=f"Training_results/{args.njets}/{args.region}/{args.preselection}")
    paths_plots = StorePathHelper(directory=f"NF_results/{args.njets}/{args.region}/{args.preselection}")
