...
ckpt_pth_fold1 = 'Categorizer_results/QCD/inclusive/fold1/2026-02-19/0_16-07-16/'
ckpt_pth_fold2 = 'Categorizer_results/QCD/inclusive/fold2/2026-02-19/0_16-07-58/'
...
def main() -> None:
    ...
    model2 = load_model(INPUT_DIM, ckpt_pth_fold1 + 'model_checkpoint.pth', device)
    model2 = load_model(INPUT_DIM, ckpt_pth_fold1 + 'model_checkpoint.pth', device)
    ...
    probs_datat1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 0], device)
    probs_datav1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 0], device)

    probs_Wjetst1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 1], device)
    probs_Wjetsv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 1], device)

    probs_diboson_Jt1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 2], device)
    probs_diboson_Jv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 2], device)

    probs_diboson_Lt1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 3], device)
    probs_diboson_Lv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 3], device)

    probs_DYjets_Jt1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 4], device)
    probs_DYjets_Jv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 4], device)

    probs_DYjets_Lt1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 5], device)
    probs_DYjets_Lv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 5], device)

    probs_ST_Jt1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 6], device)
    probs_ST_Jv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 6], device)

    probs_ST_Lt1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 7], device)
    probs_ST_Lv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 7], device)

    probs_ttbar_Jt1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 8], device)
    probs_ttbar_Jv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 8], device)

    probs_ttbar_Lt1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 9], device)
    probs_ttbar_Lv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 9], device)

    probs_embeddingt1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 10], device)
    probs_embeddingv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 10], device)
                                                              
    probs_nFFt1 = predict_probabilities(model2, train2.X.ss[train2.process.ss > 1], device)
    probs_nFFv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss > 1], device)

    probs_qcdt1 = predict_probabilities(model2, train2.X.ss[train2.process.ss == 0], device)
    probs_qcdv1 = predict_probabilities(model2, val2.X.ss[val2.process.ss == 0], device)
    
    

    probs_Wjets = torch.concat([probs_Wjetst1, probs_Wjetsv1], dim = 0).detach().cpu().numpy()
    probs_data = torch.concat([probs_datat1, probs_datav1], dim = 0).detach().cpu().numpy()
    probs_diboson_J = torch.concat([probs_diboson_Jt1, probs_diboson_Jv1], dim = 0).detach().cpu().numpy()
    probs_diboson_L = torch.concat([probs_diboson_Lt1, probs_diboson_Lv1], dim = 0).detach().cpu().numpy()
    probs_ST_J = torch.concat([probs_ST_Jt1, probs_ST_Jv1], dim = 0).detach().cpu().numpy()
    probs_ST_L = torch.concat([probs_ST_Lt1, probs_ST_Lv1], dim = 0).detach().cpu().numpy()
    probs_DYjets_J = torch.concat([probs_DYjets_Jt1, probs_DYjets_Jv1], dim = 0).detach().cpu().numpy()
    probs_DYjets_L = torch.concat([probs_DYjets_Lt1, probs_DYjets_Lv1], dim = 0).detach().cpu().numpy()
    probs_ttbar_J = torch.concat([probs_ttbar_Jt1, probs_ttbar_Jv1], dim = 0).detach().cpu().numpy()
    probs_ttbar_L = torch.concat([probs_ttbar_Lt1, probs_ttbar_Lv1], dim = 0).detach().cpu().numpy()
    probs_embedding = torch.concat([probs_embeddingt1, probs_embeddingv1], dim = 0).detach().cpu().numpy()
    probs_nFF = torch.concat([probs_nFFt1, probs_nFFv1], dim = 0).detach().cpu().numpy()
    ...
    probs_qcd = torch.concat([probs_qcdt1, probs_qcdv1], dim = 0).detach().cpu().numpy()
    ...
    weights_qcd = torch.concat([weights_qcd_train2, weights_qcd_val2], dim = 0).detach().cpu().numpy()
    ...
    if args.bins == 'equi_populated':
        fig.savefig(f'plots/results_training_equi_QCD_fold2.png')
        fig.savefig(f'plots/results_training_equi_QCD_fold2.pdf')
    elif args.bins == 'uniform':
