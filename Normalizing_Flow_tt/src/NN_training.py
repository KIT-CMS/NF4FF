...
from classes.Dataclasses import RealNVP_config, Config
...
def train_process(
    ...
    logger.info(
        "%s DR events=%d | train(AR=%d, SR=%d) | val(AR=%d, SR=%d) | prior AR/SR=%.6f",
        spec.name,
        prep['data_dr_size'],
        prep['n_train_ar'],
        prep['n_train_sr'],
        ...
        for epoch in range(1, config.n_epochs + 1):
            ...
            if avg_val_loss < best_val_loss:
                ...
                checkpoint = {
                    ...
                    'training_config': {
                        'base_config_path': '../configs/config_NF.yaml',
                        'seed': SEED,
                        'test_size': test_size,
                        'random_state': random_state,
                        'hidden_dim': hidden_dim,
...
def main():
    ...
    model_root_dir = Path(args.output_root_base) / 'binary_classifier' / f"training_{training_model_tag}"
    ...
    logger.info("Completed Wjets and QCD SR/AR binary-classifier training.")
