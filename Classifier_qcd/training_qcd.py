...
def get_ff_dataset_with_qcd_weights_ss(
    ...
    apply_qcd_to_class_weights: bool = False,
    ...
    for njets_group in (njets_groups if subtract_njets_based else ((0, 1000),)):
        ...
        for sr_value in (True, False):
            ...
            if apply_qcd_to_class_weights and hasattr(_dataset, "class_weights"):
...
def main():
    ...
    data_path = Path(__file__).parent / 'data' / 'data_complete.feather'
    ...
    for fold, train_pt, val_pt  in zip(['fold1', 'fold2'], [train_pt1, train_pt2], [val_pt1, val_pt2]):
        ...
        for epoch in range(config.n_epochs):
            ...
            w_train = train_pt.weights.ss * train_pt.class_weights.ss

            X_val = val_pt.X.ss
            y_val = val_pt.Y.ss
            w_val = val_pt.weights.ss * val_pt.class_weights.ss
